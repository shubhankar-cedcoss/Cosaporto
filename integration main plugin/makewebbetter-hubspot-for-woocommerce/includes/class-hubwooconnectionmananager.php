...
class HubWooConnectionMananager {
	...
	public function create_customer_by_email( $email, $fname, $lname ) {
		...
		$message         = __( 'Creating New Contact', 'hubspot-for-woocommerce' );
	...
	public function update_existing_products( $hubwoo_ecomm_pro_id, $properties ) {
		...
		$hapikey      = '0830c7b2-1763-4797-b6ca-4b25fc0f7a3d';
		$url          = '/crm-objects/v1/objects/products/' . $hubwoo_ecomm_pro_id . '?hapikey=' . $hapikey;
		//$headers      = $this->get_token_headers();
		$headers      = array(
			'Content-Type'  => 'application/json',
		);
	 ...
	 * @param array  $deal_details details of the deal.
	 */
	public function create_company_data( $company_details ) {
		$hapikey = '0830c7b2-1763-4797-b6ca-4b25fc0f7a3d';
		$url = '/companies/v2/companies?hapikey=' . $hapikey;

		$headers = ["Content-Type" => "application/json"];
		
		$company_details = json_encode( $company_details );
		$response     = wp_remote_request(
			$this->base_url . $url,
			array(
		...
		$message = __('Creating New Company','hubwoo');
		$this->create_log( $message, $url, $response );
		return $parsed_response;		
	}	
	...
	/**
	 * Creating association between deals and companies.
	 *
	 * @since    1.0.0
	 * @param string $deal_id id of the deal.
	 * @param array  $comp_id id of the company.
	 */
	public function create_company_associations ( $deal_id, $comp_id ) {
	//echo $deal_id . $comp_id;
		$hapikey = '0830c7b2-1763-4797-b6ca-4b25fc0f7a3d';
		$url = '/crm-associations/v1/associations?hapikey=' . $hapikey;
		//$headers = $this->get_token_headers();
		$headers = ["Content-Type" => "application/json"];
		$request = array( 
			"fromObjectId" => $comp_id, 
			"toObjectId" => $deal_id, 
			"category" => "HUBSPOT_DEFINED", 
			"definitionId" => "6" 
		);
